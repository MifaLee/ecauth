...
EC_REDIRECT_URI=https://tools.workec.com/ecauth/auth/callback
```
...
```text
https://tools.workec.com/ecauth/auth/callback
```
...
这一段已存在于 [nginx-tools.workec.com.conf](nginx-tools.workec.com.conf)。

---

## 7. Nginx 配置

当前仓库中的 [nginx-tools.workec.com.conf](nginx-tools.workec.com.conf) 已包含认证中心所需的两段关键配置。
...
```bash
curl -sf http://127.0.0.1:3008/ecauth/health
curl -I https://tools.workec.com/ecauth/
```
...
```text
https://tools.workec.com/ecauth/admin
```
...
```text
https://tools.workec.com/ecauth/auth/login?projectKey=sales-crm&returnTo=https://crm.example.com/auth/callback
```
